...
try{
	...
	if(wait == false) {
		...
		out.print("Congratulations, your ticket has been reserved. Thank you for using our service."); %>
		<br/>
		<a href="home.jsp">Return Home</a>
		<%
	}
...
//For when Customer Rep makes reservations
		...
		for(int i = 0; i < fnum.length; i++) {
			...
			totalfare = totalfare + res.getFloat("price");
			if(res2.getInt("count") >= res3.getInt("numseats_"+cl)) {
				out.print("Sorry, but it seems Flight " + fnum[i] + " of Airline " + id[i] + "is full. The customer will be placed on the waiting list for this flight. The other flights will be cancelled."); %>
				<br/>
				<a href="home.jsp">Return Home</a>
				<%
				wait = true;
		...
		if(wait == false) {
			...
			out.print("Congratulations, the ticket has been reserved for the Customer."); %>
			<br/>
